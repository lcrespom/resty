...
// -------------------- App setup --------------------
const config = (0, cli_1.getConfig)();
...
function setupPeriodicWrite() {
    ...
    setInterval(_ => {
        for (let fname of Object.keys(changed)) {
            fs.writeFile(config.dataDir + fname + '.json', JSON.stringify(files[fname], null, 2), (err) => console.dir(err));
        }
        changed = {};
    }, config.writeDelay);
}
...
function handleGetAll(req, res) {
    getJsonFile(req.params.file)
        ...
        let data = json;
        if (req.query)
            data = (0, search_1.search)(req.query, json);
